...
class LineBot extends EventEmitter {
  ...
  parse(body) {
    ...
    body.events.forEach(function(event) {
      debug('%O', event);
      event.reply = function (message, emojis = null) {
        return that.reply(event.replyToken, message, emojis);
      };
      if (event.source) {
  ...
  static createMessages(message, emojis = null) {
    if (typeof message === 'string') {
      if (emojis === null) {
        return [{ type: 'text', text: message }];
      } else {
        return [{ type: 'text', text: message, emojis: emojis }];
      }
    }
  ...
  reply(replyToken, message, emojis = null) {
    const url = '/message/reply';
    const body = {
      replyToken: replyToken,
      messages: LineBot.createMessages(message, emojis)
    };
    debug('POST %s', url);
    debug('%O', body);
