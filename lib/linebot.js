...
class LineBot extends EventEmitter {
  ...
  getTotalReplyMessages(date) {
    ...
    const url = `/message/delivery/reply?date=${date}`;
    debug('GET %s', url);
    return this.get(url).then(res => res.json()).then((result) => {
      debug('%O', result);
      return result;
  ...
  getTotalPushMessages(date) {
    const url = `/message/delivery/push?date=${date}`;
    debug('GET %s', url);
    return this.get(url).then(res => res.json()).then((result) => {
      debug('%O', result);
      return result;
  ...
  getTotalBroadcastMessages(date) {
    const url = `/message/delivery/broadcast?date=${date}`;
    debug('GET %s', url);
    return this.get(url).then(res => res.json()).then((result) => {
      debug('%O', result);
      return result;
  ...
  getTotalMulticastMessages(date) {
    const url = `/message/delivery/multicast?date=${date}`;
    debug('GET %s', url);
    return this.get(url).then(res => res.json()).then((result) => {
      debug('%O', result);
      return result;
...
function yesterday() {
  ...
  const tempDateString = tempDate.toLocaleString("en-US", {
    timeZone: "Asia/Tokyo",
    day: "2-digit",
    month: "2-digit",
    year: "numeric"
  });
  const yesterday = tempDateString.substr(6, 4) +
      tempDateString.substr(0, 2) +
      tempDateString.substr(3, 2);
}
