...
class LineBot extends EventEmitter {
	...
	constructor(options) {
		...
		this.headers = {
			...
			'Authorization': 'Bearer gRI9J+Rta1aNKAunotpLPNkpmiXFr92+d3zvt5yu8KPT9wcZb3ls7Cr2a7kNr03l1qeMNWZOM94fNgkWx4amoxH/aISig/ME07JlIGmsAuj0RMldvZov3jN+4Zedom28WOmjv2gjINjTyl72c/4TfgdB04t89/1O/w1cDnyilFU=' + this.options.channelAccessToken
		};
	...
	parse(body) {
		...
		body.events.forEach(function (event) {
			/*let profilePromise, contentPromise;
			if (event.type === 'message' && typeof event.source === 'object' && event.source.type === 'user') {
				profilePromise = fetch(that.endpoint + '/profile/' + event.source.userId, { method: 'GET', headers: that.headers }).then(function (res) {
					return res.json();
				}).then(function (json) {
			...
			if (event.message) {
				event.message.content = function () {
					return fetch(that.endpoint + '/message/' + event.message.id + '/content/' + event.source.userId, { method: 'GET', headers: that.headers }).then(function(res) {
						return res.buffer();
					});
				};
			}
			//Promise.all([profilePromise]).then(function () {
	...
	reply(event, message) {
		var body = { replyToken: event.replyToken };
		if (typeof message === 'string') {
			body.messages = [{ type: "text", text: message }];
		} else {
			body.messages = [message];
		}
		return fetch(this.endpoint + '/message/reply', { method: 'POST', headers: this.headers, body: JSON.stringify(body) })
	}
