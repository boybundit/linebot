...
class LineBot extends EventEmitter {
	...
	verify(rawBody, signature) {
		...
		const hash = crypto.createHmac('sha256', this.options.channelSecret).update(rawBody, 'utf8').digest('base64');
		console.log(hash);
